...
# --------------------------------------------------------------
mount_iso() {
    local mount_point="/mnt/xcpng_installer_$$"
    mkdir -p "$mount_point"
    if mountpoint -q "$mount_point"; then
        ...
        read -r
        run_as_root mount /dev/cdrom "$mount_point" || { error "Failed to mount /dev/cdrom on $mount_point"; exit 1; }
        if ! mountpoint -q "$mount_point"; then
...
run_as_root bash "$mp/Linux/install.sh"
